...
import { Users, Building2, AlertCircle, Trash2, Ban, CheckCircle, Loader2, FileText, ChevronLeft, ChevronRight } from "lucide-react";
...
export const AdminDashboard = () => {
    ...
    // Pagination State
    const [page, setPage] = useState({ users: 1, complaints: 1, updates: 1 });
    const [totalPages, setTotalPages] = useState({ users: 1, complaints: 1, updates: 1 });
    const [totalItems, setTotalItems] = useState({ users: 0, complaints: 0, updates: 0 });
    const limit = 10;
    ...
    const loadUsers = async () => {
        try {
            ...
            const res = await api.get("/api/v1/admin/users", {
                params: { page: page.users, limit }
            });
            setUsers(res.data.data.users);
            setTotalPages(prev => ({ ...prev, users: res.data.data.totalPages }));
            setTotalItems(prev => ({ ...prev, users: res.data.data.totalUsers }));
        } catch (e) {
    ...
    const loadComplaints = async () => {
        try {
            ...
            const res = await api.get("/api/v1/admin/complaints", {
                params: { page: page.complaints, limit }
            });
            setComplaints(res.data.data.complaints);
            setTotalPages(prev => ({ ...prev, complaints: res.data.data.totalPages }));
            setTotalItems(prev => ({ ...prev, complaints: res.data.data.totalComplaints }));
        } catch (e) {
    ...
    const loadUpdates = async () => {
        try {
            ...
            const res = await api.get("/api/v1/admin/updates/pending", {
                params: { page: page.updates, limit }
            });
            setUpdates(res.data.data.updates);
            setTotalPages(prev => ({ ...prev, updates: res.data.data.totalPages }));
            setTotalItems(prev => ({ ...prev, updates: res.data.data.totalUpdates }));
        } catch (e) {
    ...
    const PaginationControls = ({ tab }) => (
        <div className="flex items-center justify-between mt-4 border-t border-slate-200 pt-4">
            <button
                onClick={() => handlePageChange(tab, Math.max(1, page[tab] - 1))}
                disabled={page[tab] === 1 || loading}
            ...
            <span className="text-sm text-slate-600 font-medium">
                Page {page[tab]} of {totalPages[tab]} ({totalItems[tab]} items)
            </span>

            <button
                onClick={() => handlePageChange(tab, Math.min(totalPages[tab], page[tab] + 1))}
                disabled={page[tab] === totalPages[tab] || loading}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                Next
        ...
        <div className="max-w-6xl mx-auto space-y-6 p-6">
            ...
            {activeTab === "complaints" && (
                <>
                    ...
                    <PaginationControls tab="complaints" />
                </>
            )}
            ...
            {activeTab === "updates" && (
                <>
                    ...
                    <PaginationControls tab="updates" />
                </>
            )}
        </div>
    );
